...
output_dim = 3 #three outputs per windows representing likelihood of note direction being down, no change, or up
...
class EEGWindowGenerator(Sequence): #initially the model.fit call was overloading colab gpu vram, but calling this class on the data before allows it to be chunked into more doable sizes
    ...
    def __getitem__(self, idx):
        ...
        mask_batch = self.mask[batch_inds].squeeze()
...
def create_contour_labels(length, midi_notes):

    note_samples = np.array([int(t * 128) for t, _ in midi_notes], dtype=np.int64)

    num_windows = 1 + (length - original_window_samples) // step_samples
    starts = np.arange(0, num_windows * step_samples, step_samples)
    ends = starts + original_window_samples
    ...
    for i, (start, end, center) in enumerate(zip(starts, ends, centers)):
        ...
        in_window = np.where((note_samples >= start) & (note_samples < end))[0]

        if len(in_window) == 0:
            continue

        pitches = np.array([p for _, p in midi_notes], dtype=np.int64)
        center_dists = np.abs(note_samples[in_window] - center) #pick note closest to center of window, we found this resulted in most recognizable midi generation
        idx1 = in_window[np.argmin(center_dists)]
        pitch1 = pitches[idx1]
        time1 = note_samples[idx1]
        candidates = np.where(note_samples > time1)[0]
        ...
        pitch2 = pitches[idx2]
        contour = int(np.sign(pitch2 - pitch1)) + 1  #creates label of 0 for down, 1 for no change, and 2 for up
        y[i] = contour
        mask[i] = 1.0
...
def pretrain_dataset(subjects, note_dict):
    ...
    for subj in subjects:
        ...
        for trial_idx in range(labels.shape[0]):
            ...
            y, mask = create_contour_labels(length, notes)
    ...
    np.savez("/content/drive/MyDrive/global_norm_contour.npz", mean=mean, std=std)
...
def pretrain_eegnet(X_listen, y_listen, mask_listen, subjects_list):
    ...
    out = Dense(output_dim, activation='softmax')(base.get_layer('flatten').output)
    model = Model(inputs=base.input, outputs=out)
    model.compile(optimizer=Adam(1e-3), loss = 'sparse_categorical_crossentropy')

    checkpoint = ModelCheckpoint("/content/drive/MyDrive/pretrain_contour.keras", save_best_only=True, monitor='val_loss', mode='min')
...
def build_cnn_rnn(chans, samples, subject_list):
    base = EEGNet(nb_classes=output_dim, Chans=chans, Samples=samples)
    base.load_weights("/content/drive/MyDrive/pretrain_contour.keras")
...
X_listen, y_listen, mask_listen, subjects_list = pretrain_dataset(subjects, note_dict)
pretrain_eegnet(X_listen, y_listen, mask_listen, subjects_list)

norm = np.load("/content/drive/MyDrive/global_norm_contour.npz")
mean_pre = norm["mean"]
std_pre  = norm["std"]
...
print("chance baseline accuracy:", chance_acc)
...
for fold, (train_subj_idx, test_subj_idx) in enumerate(kf.split(subject_list)):
    ...
    ckpt = ModelCheckpoint(f"/content/drive/MyDrive/finetune_fold{fold+1}_contour.keras", save_best_only=True)
    ...
    # model.load_weights("/content/drive/MyDrive/finetune_contour.keras")
    ...
    plt.plot(y_true_all[0], label='True contour')
    plt.plot(y_pred_all[0], label='Predicted contour')
    plt.xlabel("Unmasked windows")
    ax = plt.gca()
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.ylabel("Contour direction")
    plt.yticks([0, 1, 2], ["down", "no change", "up"])
    plt.title("Example True vs. Predicted Contour")
    plt.legend()
    plt.savefig(f'/content/drive/MyDrive/exampe_prediction_fold{fold+1}_contour.svg')
    plt.savefig(f'/content/drive/MyDrive/exampe_prediction_fold{fold+1}_contour.pdf')
    # plt.show()
    plt.close()
