...
output_dim = 12 #12 outputs per window representing the likelihood of each mod12 interval size
...
def create_interval_labels(length, midi_notes):
    ...
    intervals = (np.diff(pitches) % 12).astype(np.int64)
    boundaries = note_samples[1:]
    ...
    '''
    since we can't guarantee that subjects imagined notes with perfect timing, we extend the bounds of the interval window to be a bit wider
    to represent that uncertainty, but 0.2 is a bit arbitrary here and other values haven't been tested. It might be a bit wide, since
    it increases bounds by an entire window length on each size
    '''

    boundary_margin = int(0.2 * 128)

    for boundary_sample, interval in zip(boundaries, intervals):
        low = boundary_sample - boundary_margin
        high = boundary_sample + boundary_margin

        in_boundary = np.where((centers >= low) & (centers <= high))[0]  #pick note closest to center of window, we found this resulted in most recognizable midi generation
        if len(in_boundary) == 0:
            continue

        i = in_boundary[np.argmin(np.abs(centers[in_boundary] - boundary_sample))]
        y[i] = interval
        mask[i] = 1.0
...
def pretrain_dataset(subjects, note_dict):
    ...
    for subj in subjects:
        ...
        for trial_idx in range(labels.shape[0]):
            ...
            y, mask = create_interval_labels(length, notes)
...
def pretrain_eegnet(X_listen, y_listen, mask_listen, subjects_list):
    ...
    checkpoint = ModelCheckpoint("eegnet_listening_pretrain.keras", save_best_only=True, mode='min')
...
def build_cnn_rnn(chans, samples, subject_list):
    ...
    base = EEGNet(nb_classes=output_dim, Chans=chans, Samples=samples)
    base.load_weights("eegnet_listening_pretrain.keras")
...
for fold, (train_subj_idx, test_subj_idx) in enumerate(kf.split(subject_list)):
    ...
    ckpt = ModelCheckpoint(f"finetune_fold{fold+1}.keras", save_best_only=True)
    ...
    # model.load_weights("/content/drive/MyDrive/finetune_interval.keras")
    ...
    print(f"Mean circular distance: {mean_circ:.3f}")

    print(f"Fold {fold+1} test accuracy:", acc)
    fold_scores.append(acc)

    plt.figure(figsize=(12,4))
    plt.plot(y_true_all[0], label='True interval')
    plt.plot(y_pred_all[0], label='Predicted interval')
    plt.xlabel("Unmasked windows")
    ax = plt.gca()
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.ylabel("Interval class")
    plt.yticks(list(range(12)))
    plt.title("Example True vs. Predicted Interval")
    plt.legend()
    plt.savefig('/content/drive/MyDrive/exampe_prediction_' + str(fold+1) + '_mod12.pdf')
    plt.savefig('/content/drive/MyDrive/exampe_prediction_' + str(fold+1) + '_mod12.svg')
    # plt.show()
    plt.close()
